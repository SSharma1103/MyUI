...
// This is the main landing page for your UI library.
// It imports and showcases several of the components you've built.

import React, { FC, useRef, useMemo } from 'react';
import { motion, useMotionValue, useSpring, useTransform } from 'framer-motion';
import CrosshairPreview from '@/app/components/cursor002';

// --- Reusable Components (Copied here for a self-contained example) ---
// In a real project, you would import these from their respective files.

const AnimatedButton: FC<{ href: string, text: string, variant?: 'primary' | 'secondary' }> = ({ href, text, variant = 'primary' }) => {
    ...
    return (
        <motion.a 
            href={href}
            className={`relative flex items-center justify-center w-48 h-12 px-8 py-3 overflow-hidden font-semibold transition-all duration-200 rounded-lg shadow-lg group focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-opacity-75 ${variant === 'primary' ? primaryClasses : secondaryClasses}`}
            initial="initial"
            whileHover="hover"
        >
            <motion.span variants={textVariants} transition={{ duration: 0.3, ease: "easeInOut" }}>
                {text}
            </motion.span>
            <motion.span className="absolute ml-15" variants={arrowVariants} transition={{ duration: 0.3, ease: "easeInOut" }}>
                <ArrowRightIcon />
            </motion.span>
        </motion.a>
    );
...
const SpotlightCard: FC<{title: string, description: string}> = ({ title, description }) => {
...
const RevealEffectCard: FC = () => {
    ...
    const smoothPanelX = useSpring(panelX, { stiffness: 400, damping: 40, mass: 1 });
    return (
        <motion.div 
            ref={cardRef}
        ...
        >
            <h3 className="text-4xl font-bold text-slate-600 z-10">Hover it</h3>
            <motion.div
            ...
            >
                ...
                <p className="text-slate-300 text-center">This panel slides with your cursor to reveal more information.</p>
            </motion.div>
        </motion.div>
    );
};
...
const HeroSection = () => (
    <section className="relative text-center py-20 md:py-32">
        ...
        >
            <span className="rounded-full bg-slate-800 px-3 py-1 text-sm font-semibold text-sky-400">Version 1.0 is here!</span>
        </motion.div>
        <motion.h1 
        ...
        >
            MyUI is a collection of beautifully designed, animated, and accessible components. Copy and paste them directly into your apps and customize to your heart&apos;s content.
        </motion.p>
        <motion.div 
        ...
        >
            <AnimatedButton href="/docs/introduction" text="Get Started" variant="primary" />
            <AnimatedButton href="https://github.com/SSharma1103/MyUI" text="GitHub" variant="secondary" />
        </motion.div>
    </section>
);
...
const FeaturesSection = () => (
    <section className="p-16">
        <div className="text-center mb-12">
            ...
            <p className="max-w-xl mx-auto mt-2 text-slate-400">Our philosophy is simple: give developers control.</p>
        </div>
...
const ShowcaseSection = () => (
    <section className="py-16 flex flex-col items-center">
        <div className="text-center mb-12">
            ...
            <p className="max-w-xl mx-auto mt-2 text-slate-400">Experience the interactivity of our components firsthand.</p>
        </div>
        <div className='w-190'>
        <CrosshairPreview/>
        </div>
...
// --- The Final Landing Page Component ---
